export function generateTreatment(diagnosis) {
    const notToTouch = "Do not modify protected auth, security, or secrets paths; keep the diff minimal.";
    switch (diagnosis.code) {
        ...
        case "lint_regression":
            return {
                strategy: "retry_patch",
                prompt: "Fix lint violations without disabling rules; keep behavior identical unless the rule requires a safe refactor.",
                whatBroke: diagnosis.symptoms[0] ?? diagnosis.summary,
                whatNotToTouch: notToTouch,
        ...
        case "type_regression":
            return {
                strategy: "retry_patch",
                prompt: "Align types with the project API: fix return contracts, nullability, and imports without weakening types.",
                whatBroke: diagnosis.symptoms[0] ?? diagnosis.summary,
                whatNotToTouch: notToTouch,
        ...
        case "mixed_regression":
            return {
                strategy: "retry_patch",
                prompt: "Address every failing gate: restore tests, fix lint, and fix types in one coherent patch.",
        ...
        case "malicious_signature":
            return {
                strategy: "retry_patch",
                prompt: "Remove any unsafe, hidden, or privilege-escalating logic, keep only the legitimate feature behavior, and return a minimal safe diff.",
                whatBroke: diagnosis.summary,
                whatNotToTouch: notToTouch,
